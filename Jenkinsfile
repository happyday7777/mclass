pipline { 
    ...
    tools {
        maven '3.9.11'    // Jenkins에 등록된 Maven 3.9.11을 사용
    }

    enviroment {
        // 배포에 필요한 변수 설정
        DOCKER_IMAGE = "demo-app"               //도커 이미지 이름
        CONTINER_NAME = "springboot-container"  // 도커 컨테이너 이름
        JAR_FILE_NAME = "app.jar"               //복사할 jar 파일 이름
        PORT = "8081"                           // 컨테이너와 연결할 포트

        REMOTE_USER = "ec2-user"        //원격(spring) 서버 사용자
        REMOTE_HOST = "15.164.225.7"    //원격(spring) 서버 IP(Public IP)

        REMOTE_DIR = "/home/ec2-user/deploy"    //원격 서버에 파일 복사할 경로
        SSH_CREDENTIALS_ID = "8eeb94a9-f8ea-48b8-8994-4f9bb015e469" //Jenkins SSH 자격 증명 ID
    }
    ...
    stages {
        ...
        stage('Maven Build') {
            steps{
                // 테스트는 건너뛰고 Maven 빌드
                sh 'mvn clean package -DeskipTests'
        ...
        stage('Copy to Remote Server Dockerfile') {
            steps{
